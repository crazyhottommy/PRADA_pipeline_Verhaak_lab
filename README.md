...
### Filter against Normal
There are some fusions that also occur in normal samples. We need to filter our list againest them.

For example:   TFG	 and GPR128 fusions are frequently observed in normal samples.  I put the list in the repo.  

The list is generated by running PRADA on TCGA normal control samples and you can find it in the repo. We are running PRADA on more normal samples from the TCGA project and will keep the list updated.  

### Exon expression profile for fusion pairs
Fusion events are usually associated with change of expression of exons. Exons at the 5' of fusion gene A and Exons at the 3' of fusion gene B should have higer expression than the rest of the exons. One exception is the balanced fusions, where 5' exons of gene B are fused with 3' exons of gene A.   
See below:  

![](fusion_exons.png)

So, we can make exon expression plots to examin whether the fusions are real to not.  
